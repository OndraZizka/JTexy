...
final class TexyImageModule extends TexyModule
{
	...
	public function factoryImage($content, $mod, $tryRef = TRUE)
          // @ String $mod is only used to set the modifier - can be done outside.
	{
    // @ This is the only thing preventing this metod to be static.
		$image = $tryRef ? $this->getReference(trim($content)) : FALSE;

    // @ ImageModule.createImage( String def )
		if (!$image) {
			$tx = $this->texy;
