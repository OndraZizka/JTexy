...
class Texy extends TexyObject
{
    ...
    const REVISION = '8da0750 released on 2009-09-26';

    // types of protection marks
    /* @ Ugly construct, can we find anything better? */
    ...
    // HTML modes
    /* @ Isn't this a good candidate for enum? */
    ...
    /* @ We should create a Configurator that could read some sort
     * of a configuration file and create an appropriate Texy object (factory?)
     */
     ...
     * @ Usage only at LineParser, the _ variant has swapped keys/values
     * for fast lookup.
     */
     ...
     * @var array of ('handler' => callback
     *                'pattern' => regular expression)
     * @ Usage only at BlockParser, the _ variant has swapped keys/values
     * for fast lookup.
     */
    ...
    /** @var array */
    /* @ Typography and LongWords module get registered here, they are used in
     * stringToHtml()
     */
    ...
    /** @var bool */
    /* @ We MUST NOT need anything like this in Java */
    private $processing;

    /** @var array of events and registered handlers */
    /* @ Contains categories: script, htmlComment, htmlTag, image, beforeParse,
     * phrase, newReference, linkReference, linkEmail, linkURL, emoticon,
     * paragraph, block, beforeBlockParse, figure, horizline, heading,
     * afterParse, postProcess
     * This seems like there will be a great bunch of interfaces.
     */
    ...
    // @ TypographyModule, LongWordsModule.
    // @ Implemented as RegexpInfo.Type.POST_LINE? Or AfterLineEvent?
    final public function registerPostLine($handler, $name)
    {
        // @ PHP crap - handler is Array($obj, "methodName").
        if (!is_callable($handler)) {
            $able = is_callable($handler, TRUE, $textual);
            throw new InvalidArgumentException("Handler '$textual' is not " . ($able ? 'callable.' : 'valid PHP callback.'));
        }
    ...
    public function process($text, $singleLine = FALSE)
    {
        ...
        // @ We will do this other way - 1) class based, 2) checked at the parse time.
        // select patterns
     ...
     * 
     * @ This function is never used, its functionality is covered by postHandler
     * from Typography and LongWords modules.
     * @ It's most likely intended to be used instead of process().
     */
    ...
    final public function stringToHtml($s)
    {
        ...
        foreach ($this->postHandlers as $name => $handler) {
            ...
            foreach ($blocks as $n => $s) {
                if ($n % 2 === 0 && $s !== '') {  // @ I did this by a trick with -2 in MatchWithOffset.
    ...
    /**
     * Converts internal string representation to final HTML code in UTF-8.
     * @ Bullshit, this converts final HTML to Text-only version.
     * 
     * @return string
     */
    ...
    // @ Will be done using TexyParserEvent and listeners.
    /**
    ...
    // @ Texy listeners - called upon various occassions, see wiki.
    // @ Will be done using TexyParserEvent and listeners.
    /**
